...
contract Bank is ERC20, ReentrancyGuard, Ownable {
    ...
    string public name = "Interest Bearing BUSD";
    string public symbol = "iBUSD";
    ...
    function pendingInterest(uint256 msgValue) public view returns (uint256) {
        if (now > lastAccrueTime) {
            ...
            uint256 balance = token.myBalance().sub(msgValue);
        ...
        onlyEOA transferTokenToVault(principalAmount) accrue(principalAmount) nonReentrant
    {
        ...
        uint256 back;
        {
            ...
            require(sendBEP20 <= token.myBalance(), "Vault::work:: insufficient funds in the vault");
            uint256 beforeBEP20 = token.myBalance().sub(sendBEP20);
            SafeToken.safeTransfer(token, goblin, sendBEP20);
            Goblin(goblin).work(id, msg.sender, debt, data);
            back = token.myBalance().sub(beforeBEP20);
        }
    ...
    function kill(uint256 id) external onlyEOA accrue(0) nonReentrant {
        ...
        if (prize > 0) {
            ...
            SafeToken.safeTransfer(token, rewardTo, prize);
        }
        uint256 left = rest > debt ? rest - debt : 0;
        if (left > 0) SafeToken.safeTransfer(token, pos.owner, left);
